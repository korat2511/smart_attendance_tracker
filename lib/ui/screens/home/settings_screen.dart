...
import 'package:smart_attendance_tracker/services/storage_service.dart';
...
import 'package:smart_attendance_tracker/ui/screens/auth/login_screen.dart';
import 'package:smart_attendance_tracker/ui/providers/subscription_provider.dart';
import 'package:smart_attendance_tracker/ui/screens/subscription/subscription_screen.dart';
import 'package:smart_attendance_tracker/services/version_check_service.dart';
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _openPlayStore([String? url]) async {
    final uri = Uri.parse(url ?? 'https://play.google.com/store/apps/details?id=com.originlab.attendex');
    if (await canLaunchUrl(uri)) {
      await launchUrl(uri, mode: LaunchMode.externalApplication);
    }
  }
  ...
  Future<void> _checkForUpdates(BuildContext context) async {
    if (_updateAvailable == true) {
      _openPlayStore(_updateStoreUrl);
      return;
    }
    ...
    setState(() {
      ...
      _updateStoreUrl = result.androidUrl;
    });
    if (result.updateAvailable) {
      SnackbarUtils.showSuccess('New version available. Tap again to open Play Store.');
    }
  }

  String _updateSubtitle() {
    if (_isCheckingUpdates) return 'Checking...';
    if (_updateAvailable == true) return 'New version available';
    if (_updateAvailable == false) return 'You have up to date version';
    return 'Tap to check';
  }
